...
public final class FilterGameTests {
	...
	public static void lockedInputAcceptsItsOriginalIngredientAfterReload(GameTestHelper helper) {
		...
		helper.assertTrue(!reloaded.isItemValid(0, new ItemStack(Items.PORKCHOP)), "locked input accepted the wrong item");
		helper.assertTrue(!reloaded.isItemValid(1, new ItemStack(Items.BEEF)), "locked empty input accepted an item");
		helper.succeed();
	}
	...
	public static void strictFiltersStillDistinguishPotionContents(GameTestHelper helper) {
		...
		helper.assertTrue(!filter.test(harming), "filter ignored differing potion components");
